...
import { PREVIEW_PAGE_LIMIT } from "../../constants/extraction.js";
import type {
  ExtractionOptions,
  ExtractionResult,
  LocatedRegion,
...
import { refineRegionBox } from "./refine.js";
...
export class ExtractionService {
  async extract(file: UploadedFile, options: ExtractionOptions): Promise<ExtractionResult> {
  ...
  private async runStrategy(
    ...
    warnings: string[],
  ): Promise<{ regions: LocatedRegion[]; strategyUsed: StrategyName }> {
    ...
    if (!availability.ok) {
      ...
      warnings.push(
        `${options.strategy} strategy unavailable (${availability.reason ?? "not configured"}); used the heuristic engine instead.`,
      );
    ...
    try {
      ...
      if (options.strategy === "heuristic") throw error;
  ...
  private async applyOcrFallback(
    doc: RasterDocument,
    regions: LocatedRegion[],
    warnings: string[],
  ): Promise<{ regions: LocatedRegion[]; ocrUsed: boolean }> {
    try {
      ...
      const ocrRegions = locateFromWords(
        toPageWords(firstText, true, first.index === last.index),
        toPageWords(lastText, first.index === last.index, true),
      );
      const merged = regions.map((region) => {
        if (region.detected) return region;
      ...
      if (recovered) warnings.push("Used OCR fallback to recover one or more regions.");
  ...
  private async buildRegions(
    ...
    for (const type of REGION_TYPES) {
      ...
      const refinedBox = await this.refine(page.png, region.box, page.width, page.height, type);
      const image = await cropRegion(page.png, refinedBox, page.width, page.height, options.format);
      results.push({
        type,
        detected: true,
  ...
  private async refine(
    ...
    pageWidth: number,
    pageHeight: number,
    type: RegionResult["type"],
  ): Promise<NormRect> {
    try {
      return await refineRegionBox(png, box, pageWidth, pageHeight, type);
    } catch {
      return box;
    }
  }

  private async buildPreviews(doc: RasterDocument): Promise<PreviewImage[]> {
