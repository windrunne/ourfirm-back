...
export function rectIsEmpty(rect: NormRect): boolean {
  return rect.width <= 0 || rect.height <= 0;
}

export function toPixelRect(rect: NormRect, pageWidth: number, pageHeight: number): PixelRect {
...
export function bandRect(top: number, bottom: number): NormRect {
...
export function padRect(rect: NormRect, pad: number): NormRect {
...
export function offsetRectWithinBand(local: NormRect, band: NormRect): NormRect {
  return normalizeRect({
