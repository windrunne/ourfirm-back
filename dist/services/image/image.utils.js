...
export async function toGrayField(png, maxWidth = 700) {
...
export async function regionGrayField(png, rect, pageWidth, pageHeight, maxWidth = 600) {
...
export function inkRatio(field, threshold) {
    let dark = 0;
    for (let i = 0; i < field.data.length; i += 1) {
        if (field.data[i] < threshold)
            dark += 1;
    }
    return field.data.length === 0 ? 0 : dark / field.data.length;
}
export async function regionInkRatio(png, rect, pageWidth, pageHeight, threshold) {
    const px = toPixelRect(rect, pageWidth, pageHeight);
    const cropped = await sharp(png).extract(px).greyscale().raw().toBuffer();
    let dark = 0;
    for (let i = 0; i < cropped.length; i += 1) {
        if (cropped[i] < threshold)
            dark += 1;
    }
    return cropped.length === 0 ? 0 : dark / cropped.length;
}
export async function cropRegion(png, rect, pageWidth, pageHeight, format) {
    const px = toPixelRect(rect, pageWidth, pageHeight);
    const extracted = sharp(png).extract(px).flatten({ background: "#ffffff" });
    const encoded = format === "jpeg"
        ? extracted.jpeg({ quality: 92 })
        : extracted.png({ compressionLevel: 9 });
...
export async function makePreview(png, maxWidth, pageNumber) {
...
export function toDataUrl(buffer, format) {
